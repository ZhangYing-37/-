...
<div style="margin: 5% 5%; width: 450px; height: 200px;">
    <form class="layui-form layui-form-pane" action="/updateSubjectPD" method="post">
        ...
        <div class="layui-form-item layui-form-text">
            <c:if test="${sessionScope.thisSubject.s_code ne null}">
                <label class="layui-form-label">（第${sessionScope.thisSubject.s_code}题）判断题题目：</label>
            ...
            <c:if test="${sessionScope.thisSubject.s_code eq null}">
                <c:if test="${sessionScope.thisPaper.subjectList.get(i).sj_id eq null}">
                    <label class="layui-form-label">（第${sessionScope.thisPaper.subjectList.size()}题）判断题题目：</label>
                ...
                <c:if test="${sessionScope.thisPaper.subjectList.get(i).sj_id ne null}">
                    <label class="layui-form-label">（第${sessionScope.thisPaper.subjectList.size()+1}题）判断题题目：</label>
                    <input type="hidden" name="s_code" id="s_code" value="${sessionScope.thisPaper.subjectList.size()+1}">
                </c:if>
            </c:if>

            <div class="layui-input-block">
                <textarea name="s_name" id="s_name" placeholder="请输入判断题题目" required lay-verify="required" class="layui-textarea" maxlength="200">${sessionScope.thisSubject.s_name}</textarea>
            </div>
        </div>
            ...
            <table width="450px">
                ...
                <c:if test="${sessionScope.thisSubject.optionList.size() lt 2 or sessionScope.thisSubject eq null}">
                    <tr height="30px" id="option1">
                        <td align="left">
                            <span><input type="text" name="o_name1" id="o_name1" value="对" style="width: 200px;" required lay-verify="required"></span>
                        </td>
                        <td align="center">
                    ...
                    <tr height="30px" id="option2">
                        <td align="left">
                            <span><input type="text" name="o_name2" id="o_name2" value="错" style="width: 200px;" required lay-verify="required"></span>
                        </td>
                        <td align="center">
        ...
        <div class="layui-form-item">
            ...
            <button class="layui-btn" id="getSumbit" style="margin-left: 10%" lay-submit="" lay-filter="formPD">保存</button>
        </div>
    </form>
</div>
...
<script src="/exam/js/updateSubjectPD.js"></script>
