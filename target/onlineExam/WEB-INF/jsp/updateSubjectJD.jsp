...
<div style="margin: 5% 5%; width: 450px; height: 200px;">
    <form class="layui-form layui-form-pane" action="/updateSubjectJD" method="post">
        ...
        <div class="layui-form-item layui-form-text">
            <c:if test="${sessionScope.thisSubject.s_code ne null}">
                <label class="layui-form-label">（第${sessionScope.thisSubject.s_code}题）简答题题目：</label>
            ...
            <c:if test="${sessionScope.thisSubject.s_code eq null}">
                <c:if test="${sessionScope.thisPaper.subjectList.get(i).sj_id eq null}">
                    <label class="layui-form-label">（第${sessionScope.thisPaper.subjectList.size()}题）简答题题目：</label>
                ...
                <c:if test="${sessionScope.thisPaper.subjectList.get(i).sj_id ne null}">
                    <label class="layui-form-label">（第${sessionScope.thisPaper.subjectList.size()+1}题）简答题题目：</label>
                    <input type="hidden" name="s_code" id="s_code" value="${sessionScope.thisPaper.subjectList.size()+1}">
                </c:if>

            </c:if>
            <div class="layui-input-block">
                <textarea name="s_name" id="s_name" placeholder="请输入简答题题目" class="layui-textarea" maxlength="200">${sessionScope.thisSubject.s_name}</textarea>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" value="答案由教师批改" class="layui-input" disabled>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <button class="layui-btn" id="getSumbit" style="margin-left: 10%" lay-submit="" lay-filter="formJD">保存</button>
        </div>
    </form>
</div>
...
<script type="text/javascript">
    ...
    layui.use('form', function(){
        var form = layui.form
        form.on('submit(formJD)', function (data) {
